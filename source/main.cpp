...
//     PARQUET_THROW_NOT_OK(parquet::arrow::WriteTable(*parquet_table, arrow::default_memory_pool(), outfile, 5));
...
arrow::Status RunMain() {
    ...
    std::shared_ptr<arrow::Schema> schema = arrow::schema({
        ...
        arrow::field("vz", arrow::float64()),
    });

    // Loop over states
    arrow::DoubleBuilder doubleBuilder;
    const size_t nStates = vehicle.n_states();
    double timeCol[nStates];
    double xCol[nStates];
    double yCol[nStates];
    double zCol[nStates];
    double vxCol[nStates];
    double vyCol[nStates];
    double vzCol[nStates];
    for (auto& state : vehicle.get_states()) {
        state.elements.convert(ElementSet::CARTESIAN, &sys);
    }
