...
OrbitalElements KeplerianVop::operator()(const Time& time, const OrbitalElements& state, const Vehicle& vehicle) const {
    ...
    */
    ...
    const quantity<day> julianDate = vehicle.get_epoch().julian_day() + time;
    ...
    const quantity<one/s>        deccdt   = h/mu*sinTA*radialPert + 1/(mu*h)*((hSquared + mu*R)*cosTA + mu*ecc*R)*tangentialPert;
    const quantity<rad/s>        dincdt   = R/h*cosU*normalPert;
    const quantity<rad/s>        dthetadt = hOverRSquared + (1/(ecc*h))*((hSquared/mu)*cosTA*radialPert - (hSquared/mu + R)*sinTA*tangentialPert);
