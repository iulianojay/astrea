...
basis_array NBodyForce::compute_force(const double& julianDate, const OrbitalElements& state, const Vehicle& vehicle, const AstrodynamicsSystem& sys) const {
    ...
    for (auto&& [name, body] : sys) {
        ...
        const basis_array radiusCentralBodyToNbody{
            radiusSunToNbody[0] + radiusCentralBodyToSun[0],
            radiusSunToNbody[1] + radiusCentralBodyToSun[1],
            radiusSunToNbody[2] + radiusCentralBodyToSun[2]
        };

        const basis_array radiusVehicleToNbody{
            radiusCentralBodyToNbody[0] - x,
            radiusCentralBodyToNbody[1] - y,
            radiusCentralBodyToNbody[2] - z
        };

        // Normalize
        const double radiusVehicleToNbodyMagnitude = math_c::normalize(radiusVehicleToNbody, 2);
        const double radiusCentralToNbodyMagnitude = math_c::normalize(radiusCentralBodyToNbody, 2);

        // Perturbational force from nth body
        const double tempA = body->get_mu()/(radiusVehicleToNbodyMagnitude*radiusVehicleToNbodyMagnitude*radiusVehicleToNbodyMagnitude);
        const double tempB = body->get_mu()/(radiusCentralToNbodyMagnitude*radiusCentralToNbodyMagnitude*radiusCentralToNbodyMagnitude);
