...
AccelerationVector SolarRadiationPressureForce::compute_force(const double& julianDate, const OrbitalElements& state,
    ...
    const quantity radialMagnitudeCenterToSun = sqrt(radiusCenterToSun[0]*radiusCenterToSun[0] +
        radiusCenterToSun[1]*radiusCenterToSun[1] + radiusCenterToSun[2]*radiusCenterToSun[2]);

    const RadiusVector radiusVehicleToSun{
        radiusCenterToSun[0] - x,
        radiusCenterToSun[1] - y,
        radiusCenterToSun[2] - z
    };
    const quantity rMagVehicleToSun = sqrt(radiusVehicleToSun[0]*radiusVehicleToSun[0] +
        radiusVehicleToSun[1]*radiusVehicleToSun[1] + radiusVehicleToSun[2]*radiusVehicleToSun[2]);

    // Solar radiation pressure
    const quantity solarRadiationPressure = SRP_1AU*(au*au)/(rMagVehicleToSun*rMagVehicleToSun); // Scale by(1AU/R)^2 for other bodies
    ...
    if (isSun) {
        ...
        const quantity refAngle = acos((radiusCenterToSun[0]*x + radiusCenterToSun[1]*y + radiusCenterToSun[2]*z)/(radialMagnitudeCenterToSun*R));
        ...
        if (refAngle1 + refAngle2 <= refAngle) { // In shadow
            ...
            const RadiusVector rP{
                -Xu*radiusCenterToSun[0]/radialMagnitudeCenterToSun,
    ...
    const quantity tempA = -solarRadiationPressure*coefficientOfReflectivity*(areaSun)/mass/rMagVehicleToSun*fractionOfRecievedSunlight;

    const AccelerationVector accelSRP{
