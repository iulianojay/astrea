...
astro::RadiusVector<astro::ECI> GroundStation::get_inertial_position(const astro::Date& date) const
{
    return _lla.get_position(date, _parent);
}

astro::VelocityVector<astro::ECI> GroundStation::get_inertial_velocity(const astro::Date& date) const
{
    ...
    const VelocityVector<ECEF> vEcef = (z.cross(rEcefPlanar).unit()) * vEcefMag; // z cross (x,y,0) give perpendicular direction vector
