...
class AtmosphericForce : public PerturbingForce {
  ...
  public:
    ...
    Perturbation compute_perturbation(const State& state, const Vehicle& vehicle) const override
    {
        ...
        const VelocityVector<r.frame> relVelocity      = { vx + y * bodyRotationRate.in(rad / s) / rad,
        ...
        const ForceVector<r.frame> forceDrag = dragForceMag * (relVelocity / relVelMag);
        ...
        const ForceVector<r.frame> forceLift = liftForceMag * (r / R); // just assume radial lift for now
