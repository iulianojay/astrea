 ...
 */
template <>
struct std::hash<std::pair<astrea::astro::ElementSet, astrea::astro::ElementSet>> {
    /**
     ...
     */
    std::size_t operator()(const std::pair<astrea::astro::ElementSet, astrea::astro::ElementSet>& p) const
    {
        auto h1 = std::hash<astrea::astro::ElementSet>{}(p.first);
        auto h2 = std::hash<astrea::astro::ElementSet>{}(p.second);
        return h1 ^ h2;
    }
};