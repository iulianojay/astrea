...
Density Earth::find_harris_priester_atmospheric_density(const State& state) const
{
    ...
    const RadiusVector<frames::earth::icrf> sunDirection = -sun2Earth.unit().force_frame_conversion<frames::earth::icrf>();
    const RadiusVector<frames::earth::icrf> bulgeDirection =
        sunDirection * DCM<frames::earth::icrf, frames::earth::icrf>::Z(LAG);
    ...
    const Unitless cosPow = (cPsi2 > 1.0e-12 * one) ? c2Psi2 * math::pow(cPsi2, (cosineExponent - 2) * one) : 0.;
    ...
    if (iter != HARRIS_PRIESTER_ATMOSPHERE.end()) {
        ...
        const Unitless rhoMin = minDensity * math::pow(nextMinDensity / minDensity, dH);
        if (is_eq_zero(cosPow)) { return rhoMin; }
        else {
            // Max exponential density interpolation
            const Unitless rhoMax = maxDensity * math::pow(nextMaxDensity / maxDensity, dH);
