...
Cartesian Moon::get_ephemeris_at(const Date& date) const override
{
    //! Number of days covered by each set of polynomial coefficients
    static constexpr Time timePerCoefficient = MoonEphemerisTable::TIME_PER_COEFFICIENT;

    // Extract components
    const std::size_t ind = get_index(date, timePerCoefficient);
    const auto& xInterp   = MoonGcrfTable::X_INTERP[ind];
    const auto& yInterp   = MoonGcrfTable::Y_INTERP[ind];
    const auto& zInterp   = MoonGcrfTable::Z_INTERP[ind];
