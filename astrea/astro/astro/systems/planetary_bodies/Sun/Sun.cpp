...
Cartesian Sun::get_ephemeris_at(const Date& date) const override
{
    //! Number of days covered by each set of polynomial coefficients
    static constexpr Time timePerCoefficient = SunEphemerisTable::TIME_PER_COEFFICIENT;

    // Extract components
    const std::size_t ind = get_index(date, timePerCoefficient);
    const auto& xInterp   = SunEphemerisTable::X_INTERP[ind];
    const auto& yInterp   = SunEphemerisTable::Y_INTERP[ind];
    const auto& zInterp   = SunEphemerisTable::Z_INTERP[ind];
