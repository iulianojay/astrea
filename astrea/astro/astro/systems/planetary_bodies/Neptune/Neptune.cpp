...
Cartesian Neptune::get_ephemeris_at(const Date& date) const override
{
    //! Number of days covered by each set of polynomial coefficients
    static constexpr Time timePerCoefficient = NeptuneEphemerisTable::TIME_PER_COEFFICIENT;

    // Extract components
    const std::size_t ind = get_index(date, timePerCoefficient);
    const auto& xInterp   = NeptuneEphemerisTable::X_INTERP[ind];
    const auto& yInterp   = NeptuneEphemerisTable::Y_INTERP[ind];
    const auto& zInterp   = NeptuneEphemerisTable::Z_INTERP[ind];
