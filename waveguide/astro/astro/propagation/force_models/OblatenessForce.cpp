...
void OblatenessForce::ingest_legendre_coefficient_file(const std::size_t& N, const std::size_t& M)
{
    ...
    std::filesystem::path path = std::string(std::getenv("WAVEGUIDE_ROOT")) + "/data/gravity_models/";
    ...
    OblatenessForce::compute_force(const Date& date, const Cartesian& state, const Vehicle& vehicle, const AstrodynamicsSystem& sys) const
{
    ...
    const quantity<one / waveguide::detail::distance_unit> oneOverR = 1.0 / sqrt(x * x + y * y + z * z);
    ...
    // Find lat and long
    const RadiusVector rEcef = eci_to_ecef(state.get_radius(), date);
    ...
    return ecef_to_eci(accelOblatenessEcef, date);
}
