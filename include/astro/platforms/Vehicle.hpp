...
concept HasGetMassUnits = requires(T vehicle) {
    { vehicle.get_mass() } -> std::same_as<mp_units::quantity<mp_units::si::unit_symbols::kg>>;
};
...
concept HasGetCoefficientOfDragUnits = requires(T vehicle) {
    { vehicle.get_coefficient_of_drag() } -> std::same_as<mp_units::quantity<mp_units::one>>;
};
...
concept HasGetCoefficientOfLiftUnits = requires(T vehicle) {
    { vehicle.get_coefficient_of_lift() } -> std::same_as<mp_units::quantity<mp_units::one>>;
};
...
concept HasGetCoefficientOfReflectivityUnits = requires(T vehicle) {
    { vehicle.get_coefficient_of_reflectivity() } -> std::same_as<mp_units::quantity<mp_units::one>>;
};
...
struct VehicleInner final : public VehicleInnerBase {
    ...
    Mass get_mass() const final {
        return get_mass_impl(_value);
    }
    ...
    Area get_ram_area() const final {
        return get_ram_area_impl(_value);
    }
    Area get_lift_area() const final {
    ...
    static Area get_ram_area_impl(const U& value) {
        return value.get_ram_area();
    }
    template <typename U>
    requires(HasGetRamArea<U> && !HasGetRamAreaUnits<U>)
    static Area get_ram_area_impl(const U& value) {
        return Area(value.get_ram_area());
    }
    ...
    static Area get_lift_area_impl(const U& value) {
        return value.get_lift_area();
    }
    template <typename U>
    requires(HasGetLiftArea<U> && !HasGetLiftAreaUnits<U>)
    static Area get_lift_area_impl(const U& value) {
        return Area(value.get_lift_area());
    }
    ...
    static Area get_solar_area_impl(const U& value) {
        return value.get_solar_area();
    }
    template <typename U>
    requires(HasGetSolarArea<U> && !HasGetSolarAreaUnits<U>)
    static Area get_solar_area_impl(const U& value) {
        return Area(value.get_solar_area());
    }
    ...
    requires(HasGetCoefficientOfDrag<U> && !HasGetCoefficientOfDragUnits<U>)
    static Unitless get_coefficient_of_drag_impl(const U& value) {
        return Unitless(value.get_coefficient_of_drag());
    }
    ...
    requires(HasGetCoefficientOfLift<U> && !HasGetCoefficientOfLiftUnits<U>)
    static Unitless get_coefficient_of_lift_impl(const U& value) {
        return Unitless(value.get_coefficient_of_lift());
    }
