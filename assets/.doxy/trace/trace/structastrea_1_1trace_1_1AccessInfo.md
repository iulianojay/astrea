

# Struct astrea::trace::AccessInfo



[**ClassList**](annotated.md) **>** [**astrea**](namespaceastrea.md) **>** [**trace**](namespaceastrea_1_1trace.md) **>** [**AccessInfo**](structastrea_1_1trace_1_1AccessInfo.md)


























## Public Attributes

| Type | Name |
| ---: | :--- |
|  bool | [**isOcculted**](#variable-isocculted)  <br> |
|  EcefRadiusVec | [**radius1to2**](#variable-radius1to2)  <br> |
|  Time | [**time**](#variable-time)  <br> |












































## Public Attributes Documentation




### variable isOcculted 

```C++
bool astrea::trace::AccessInfo::isOcculted;
```




<hr>



### variable radius1to2 

```C++
EcefRadiusVec astrea::trace::AccessInfo::radius1to2;
```




<hr>



### variable time 

```C++
Time astrea::trace::AccessInfo::time;
```




<hr>

------------------------------
The documentation for this class was generated from the following file `astrea/trace/trace/analysis/AccessAnalyzer.cpp`

