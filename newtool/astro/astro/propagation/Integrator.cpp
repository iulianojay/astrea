    ...
    Integrator::find_state_derivative(const Time& time, const OrbitalElements& state, const EquationsOfMotion& eom, Vehicle& vehicle)
{
    ...
    void Integrator::try_step(Time & time, Time & timeStep, OrbitalElements & state, const EquationsOfMotion& eom, Vehicle& vehicle)
    {
        ...
        for (size_t ii = 0; ii < nStages; ++ii) {
            ...
            if (ii == 0) {
                if (stepMethod == RK45 || stepMethod == RKF45 || stepMethod == RKF78) {
                    ...
                    for (size_t iState = 0; iState < dstate.size(); ++iState) {
                ...
                OrbitalElements sPlusKi = state;
                sPlusKi.update_from_vector(statePlusKi);
                const auto dstate = find_state_derivative(time + c[ii] * timeStep, sPlusKi, eom, vehicle);
                for (size_t iState = 0; iState < dstate.size(); ++iState) {
                    kMatrix[ii][iState] = dstate[iState];
                }
            }

            auto statePlusKi = state.to_vector();
            for (size_t jj = 0; jj < statePlusKi.size(); ++jj) {
                // Correct k value
        ...
        double maxError              = 0.0;
        std::vector<double> stateNew = state.to_vector();
        ...
        if (!useFixedStep) { check_error(maxError, stateNew, stateError, time, timeStep, state); }
            ...
            // Step time
            time += timeStep;
            for (size_t ii = 0; ii < updatedStateValues.size(); ++ii) {
                // Adding the state error improves the next guess
                stateNew[ii] += stateError[ii];
            ...
            state.update_from_vector(stateNew);
        }
    }


    void Integrator::check_error(
        const double& maxError, const std::vector<double>& stateNew, const std::vector<double> stateError, Time& time, Time& timeStep, OrbitalElements& state
    )
    {
