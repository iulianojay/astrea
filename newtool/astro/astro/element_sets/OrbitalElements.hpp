...
concept HasIterpolate =
    requires(const T elements, const Time& thisTime, const Time& otherTime, const OrbitalElements& other, const AstrodynamicsSystem& sys, const Time& targetTime)
{
    {
        elements.interpolate(thisTime, otherTime, other, sys, targetTime)
        } -> std::same_as<OrbitalElements>;
};
