...
void Vehicle::generic_ctor_impl()
{
    ...
    _mass = ptr()->get_mass();
    if (_mass < 0.0) { throw std::invalid_argument("Vehicle mass cannot be negative."); }

    _ramArea = ptr()->get_ram_area();
    if (_ramArea < 0.0) { throw std::invalid_argument("Vehicle ram area cannot be negative."); }

    _liftArea = ptr()->get_lift_area();
    if (_liftArea < 0.0) { throw std::invalid_argument("Vehicle lift area cannot be negative."); }

    _solarArea = ptr()->get_solar_area();
    if (_solarArea < 0.0) { throw std::invalid_argument("Vehicle solar area cannot be negative."); }

    _coefficientOfDrag = ptr()->get_coefficient_of_drag();
    if (_coefficientOfDrag < 0.0) { throw std::invalid_argument("Vehicle coefficient of drag cannot be negative."); }

    _coefficientOfLift = ptr()->get_coefficient_of_lift();
    if (_mass < 0.0) { throw std::invalid_argument("Vehicle coefficient of lift cannot be negative."); }
